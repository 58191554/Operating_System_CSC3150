...
// page table
extern __shared__ int pt[];

__device__ __managed__ int swap_table[STORAGE_SIZE + PHYSICAL_MEM_SIZE];
...
__global__ void mykernel(int input_size) {
	...
	int thread_id = threadIdx.x;
  	...
  	VirtualMemory vm;
  	vm_init(&vm, data, storage, pt, &pagefault_num, swap_table, PAGE_SIZE,
          	INVERT_PAGE_TABLE_SIZE, PHYSICAL_MEM_SIZE, STORAGE_SIZE,
          	PHYSICAL_MEM_SIZE / PAGE_SIZE, thread_id);

  	user_program(&vm, input, results, input_size, thread_id);
}
...
int main() {
	...
	// int input_size = load_binaryFile("./order.bin", input, STORAGE_SIZE);
	printf("input size = %d\n", input_size);
