...
def remove_cheat(path):
    ...
    for line in f:
        for word in a:
            if word in line:
                print("found cheating line:",line)
...
def test_bonus():
    os.system('''./pstree | grep --text 'test2022\|halt' > /tmp/o2''')
    os.system('''pstree -l | grep --text 'test2022\|halt'> /tmp/o1''')
    os.system('''pstree | grep --text 'test2022\|halt'> /tmp/o3''')
    os.system('''pstree -c | grep --text 'test2022\|halt'> /tmp/o4''')
    os.system('''rm -f ./pstree''')
    with open("/tmp/o1","r") as f: long_res = f.readlines()
    with open("/tmp/o3","r") as f: short_res = f.readlines()
    with open("/tmp/o4","r") as f: compact_res = f.readlines()
    try:
        with open("/tmp/o2","r") as f: stu_res = f.readlines()
    except OSError as e:
        print("failed to execute")
        return -1
    if match_output(long_res,stu_res) or match_output(short_res,stu_res) or match_output(compact_res,stu_res): 
        print("test passed.")
    else: 
